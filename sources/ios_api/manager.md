SDK中将对iOS BLE的操作封装成管理类(Manager)，包括扫描、建立连接、断开连接等操作。
...
如果使用了该参数, 那么 app 就可以支持"蓝牙状态保存和恢复"功能, 即当 app 被系统杀死时, 系统会保存那一刻蓝牙的状态，如果蓝牙的状态发生了变化, 系统会在后台唤醒 app, 并为 app 恢复之前保存的状态.  

该功能的实现, 需要先为 app 指定“Uses Bluetooth LE accessories”类型的后台模式.

该功能有以下限制：  
1.每次初始化 JumaManager 时, 该参数的值必须保持一致.  
2.不支持用户在后台管理界面主动杀死 app 的情况.  
3.不支持手机的蓝牙被关闭的情况.  
...
如果为该参数指定的值为 @YES, 当手机上的蓝牙处于关闭状态时, 系统会向用户显示一个警告对话框.  
如果为该参数指定的值为 @NO, 不论手机上的蓝牙是否处于关闭状态时, 系统都不会显示警告对话框.
...
如果为该参数指定的值是 @NO, 不管手机收到多少次信号, manager:didDiscoverDevice:RSSI 都只会被调用一次.
...
初始化成功之后，JumaManagerDelegate 中的 managerDidUpdateState: 会被调用，以此表明 JumaManager 的状态。
